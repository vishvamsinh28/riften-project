...
function Tip({ children }) {
  return (
    <div className="pointer-events-none absolute -top-1 left-1/2 z-10 hidden -translate-x-1/2 -translate-y-full whitespace-nowrap rounded-md border border-edge bg-overlay px-2 py-1 font-mono text-2xs text-ink shadow-pop group-hover:block">
      {children}
    </div>
  );
}
    ...
    <div className="flex h-24 items-end gap-px">
      {list.map((b, i) => (
        <div key={i} className="group relative flex h-full flex-1 items-end">
          <div
            className="bar-grow w-full rounded-t-[2px] bg-chart/60 transition-colors duration-150 group-hover:bg-chart"
  ...
  return (
    <div>
      <div className="flex h-28 items-end gap-[3px]">
        {list.map((d, i) => (
          <div key={d.day} className="group relative flex h-full flex-1 flex-col justify-end">
            {d.err > 0 ? (
              <div className="bar-grow w-full rounded-t-[2px] bg-bad/70" style={{ height: `${(d.err / max) * 100}%`, animationDelay: `${i * 10}ms` }} />
            ) : null}
            <div
              className={`bar-grow w-full bg-chart/60 transition-colors duration-150 group-hover:bg-chart ${d.err > 0 ? "" : "rounded-t-[2px]"}`}
      ...
      <div className="mt-1.5 flex justify-between">
        {list.map((d, i) =>
          i % labelEvery === 0 ? (
            <span key={d.day} className="text-2xs text-ink-mute">{fmtDate(d.day)}</span>
          ) : null
        )}
      </div>
    </div>
