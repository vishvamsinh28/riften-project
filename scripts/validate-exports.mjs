...
for (const e of sft.excluded) {
  if (!["retried_away", "continuation_rejected", "weak_rating"].includes(e.reason)) continue;
  if (!e.trace.response?.message) continue;
  if (!rejectedAnswers.has(e.trace.session_id)) rejectedAnswers.set(e.trace.session_id, new Set());
  rejectedAnswers.get(e.trace.session_id).add(JSON.stringify(e.trace.response.message));
}
let maskedSeen = 0;
for (const line of sft.lines) {
  ...
  line.messages.forEach((m, i) => {
    ...
    const { weight, ...bare } = m;
    if (set.has(JSON.stringify(bare))) {
      maskedSeen += 1;
...
for (const r of ["non_2xx", "truncated", "retried_away", "continuation_rejected", "weak_rating", "superseded"]) {
